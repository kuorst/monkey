...
module.exports = class Tag extends Command {
    ...
    run(message, args) {
      ...
      if(!message.member.hasPermission("ADMINISTRATOR", {checkOwner: true}))
        return message.sm("Y\:octagonal_sign: You don't have perms for that!", {type: "error"});
        const [title, description] = args.slice(2).join(" ").split("|");
        if (["add", "create"].includes(query)) {
            if (message.guild.db.findTag(args[1]))
                return message.sm(":octagonal_sign: That tag already exists!", {type: "error"});
            ...
            if (!message.guild.db.findTag(args[1]))
                return message.sm(":octagonal_sign: That tag doesn't exists!", {type: "error"});

            message.guild.db.deleteTag(args[1]);
        } else return message.channel.send(tagEmbed);
        message.guild.db.save();
        if (query !== "list")
            message.sm(`:white_check_mark: Successfully ${query.endsWith("e") ? query + "d" : query + "ed"} that tag!`);
    }
};