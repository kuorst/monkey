...
module.exports = class Logs extends Event {
	...
	async run(bot, log, data) {
		...
		const logEmbed = new BobbyEmbed(bot)["base"]("Not Found", data.message ? data.message.author : data.member.user);
		switch (log) {
			case "messageDelete":
				logEmbed
			...
			case "mute":
				logEmbed
				.setTitle(`**Mute**[Case: ${guild.db.case}]`)
				.setDescription(
					[`**Muted Member:** ${data.member.user.username}(${data.member.id})`,
						`**Muted By:** ${data.message.author.username}`,
						`**Reason:** ${data.reason}`,
						`**Time:** ${data.time}ms`,
						`**Channel:** ${data.message.channel}`
					].join("\n"));
