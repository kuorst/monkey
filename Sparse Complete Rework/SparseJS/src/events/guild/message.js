        ...
        if (![prefix, tagPrefix].some((pre) => message.content.startsWith(pre))) {
            if (random(100, 1) > 70) {
                ...
                if (message.member.db.xp > message.member.db.level * 130 + 5) {
                    await message.member.send(`Hey You've leveled up.\nYour new uwu level is:  ${message.member.db.level + 1}`);
        ...
        try {
            if (command) {
                if (!bot.checkPerms(message.member, command.userPermissions))
                    return message.sm(`:octagonal_sign: You don't have the required Permissions! Needed: ${command.userPermissions.join(" or ")}`);
                if (!bot.checkPerms(message.guild.me, command.botPermissions))
                    return message.sm(`:octagonal_sign: I don't have the required Permissions! Needed: ${command.botPermissions.join(" and ")}`);
                if (command.options.devOnly && !message.author.db.dev)
                    return message.sm(":octagonal_sign: You need Developer for this Command!", {type: "error"});
